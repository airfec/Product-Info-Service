...
const createAmenities = (start, end, counter = 1) => {
  ...
  for (start; start < end; start++) {
    const amenities = populateRandomAmenities(
      faker.random.number({ min: 4, max: 7 })
    );
...
const createRoom = (start, end) => {
  ...
  while (room--) {
    ...
    const house_rules_desc = `${faker.lorem.sentences(
      faker.random.number({ min: 1, max: 3 })
    )},`;
    const highlights = `${faker.lorem.sentences(
      faker.random.number({ min: 1, max: 3 })
    )}* ${faker.lorem.sentences(faker.random.number({ min: 2, max: 5 }))},`;

    const noOfRules = faker.random.number({ min: 1, max: 3 });
    let rules = "";
    for (let j = 0; j < noOfRules; j++) {
      rules += `${faker.lorem.sentences(
        faker.random.number({ min: 1, max: 3 })
      )}* `;
    }
    ...
    for (let j = 0; j < cancellationsLength; j++) {
      cancelationRules += `${faker.lorem.sentences(
        faker.random.number({ min: 1, max: 3 })
      )}* `;
    }
